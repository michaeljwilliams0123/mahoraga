...
const ready = (id) => ({ id, metered: false, priceUsd: 0, billingState: "verified-zero", zeroDollarStopGuaranteed: true, ready: true, capabilityCanary: { fresh: true } });
...
test("rejects metered, unknown-billing, stale-canary, and unready providers", () => {
...
test("zero-credit policy filters autonomous routes and leaves ordinary routing unchanged", () => {
  const manifest = {
    costModes: { local: ["deterministic", "local-model", "cloud-open-weight"] },
    routingPolicy: { interfaceOrder: ["deterministic-worker"], availabilityOrder: ["configured"], minimumReliability: 0 },
    workers: [
