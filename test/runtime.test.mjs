...
test("runtime serves the cockpit API and completes a health task", async (t) => {
  ...
  assert.match(html, /PRIMARY-LED · REPOSITORY-ONLY/);
  assert.match(html, /id="github-assurance-list"/);
  assert.doesNotMatch(html, /equal controller authority/i);
  ...
  assert.equal(coordination.authority.model, "primary-led-subordinate-workers");
  assert.equal(coordination.authority.rolesAreTransportOnly, false);
  ...
  assert.equal(coordination.authority.secondaryCanImplement, true);
  assert.equal(coordination.authority.secondaryCanReview, true);
  assert.equal(coordination.authority.secondaryCanMerge, false);
  assert.equal(coordination.authority.secondaryBranchPrefix, "secondary/");
