...
test("Control Center uploads, attaches, inspects, and protects a private artifact", async (t) => {
  ...
  await waitFor(async () => (await (await fetch(`${base}/api/status`)).json()).capabilities.some((item) => item.capability === "artifact.inspect" && item.availability === "healthy"));
  ...
  const createdTask = await (await fetch(`${base}/api/tasks`, {
    ...
    body: JSON.stringify({
      ...
      requestedMode: "local",
      requestedOutcome: "Inspect the attached evidence.",
  ...
  const deniedDelete = await fetch(`${base}/api/artifacts/${artifact.id}`, { method: "DELETE" });
