...
Once an external dedicated actor is provisioned, the manifest may be changed through the normal reviewed repository process to `dedicated-actor` with its exact public actor identity. Secrets, tokens, private keys, or ChatGPT credentials must never be stored in the manifest.
