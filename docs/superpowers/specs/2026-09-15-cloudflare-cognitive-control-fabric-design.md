...
- PR `#543` merged and establishes the current cockpit/readiness/single-brain UI baseline; later implementation must preserve it as an independent lane.
- PR `#550` merged the owner-gateway same-origin mutation boundary; later Cloudflare work must preserve its fail-closed behavior.
- Issue `#552` tracks a canonical Railway state where `/api/live` is 200 but `/api/ready` is 503; liveness must not be promoted as readiness.
...
## 23. Interaction with merged PR #543

Merged PR #543 owns the cockpit/readiness/single-brain UI convergence baseline. This design must not duplicate or rewrite that UI scope.

Implementation derived from this spec must start from current `main`, preserve #543's single-brain semantics and #550's same-origin mutation boundary, and surface new Cloudflare/control state only through existing bounded diagnostic surfaces when needed.

No implementation PR should replay #543's merged UI changes or bundle #551's separate observational UI work merely to show progress.
