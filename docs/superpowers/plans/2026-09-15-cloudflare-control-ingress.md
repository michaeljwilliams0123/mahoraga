...
- Do not duplicate merged PR #543 UI scope; CF-1 changes gateway/deployment contracts only.
- Windows production remains locked to 3.6.0; do not activate 7.0.0-alpha.2 on Windows.
- Preserve the same-origin mutation boundary merged in PR #550: cross-origin mutations fail closed and trusted mutations are rewritten only to the canonical Railway upstream.
- Treat issue #552 independently: `/api/live` success is not `/api/ready` proof, and CF-1 must not mask a concrete readiness dependency or reason code.
...
PR #543 is merged and establishes the current UI baseline. Before creating the implementation branch:
...
Open a PR to `main` containing only CF-1 files. Do not bundle #543's merged UI files, #551's separate observational UI scope, or generated runtime reports.
