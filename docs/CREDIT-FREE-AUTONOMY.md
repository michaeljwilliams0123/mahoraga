...
loopback invoke, and unattended cycle so a missing file cannot silently drop
the slow loop or the $0 generate path.
...
- Self-healer restore covers `local-reasoner-channel`, `credit-free-skill-compound`,
  `local-reasoner-generate`, `local-reasoner-loopback-invoke`, and
  `unattended-credit-free-cycle`.
