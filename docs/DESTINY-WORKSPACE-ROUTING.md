...
Owner issue command: \`/mahoraga receive workspace-agent sec-<id> destiny-workspace\`. Policies are \`destiny-workspace\`, \`mike-primary\`, \`balanced\` (Destiny first), and \`overflow\` (Mike first). The legacy command without a policy defaults to \`balanced\`.
