...
test("connected transport is degraded when assistant.respond is not routable", () => {
  const state = deriveBrainRouteState(true, [{ capability: "assistant.respond", routable: false, routingReason: "provider.gap" }]);
  assert.equal(state.kind, "capability-unavailable");
  assert.equal(brainReadiness(state), "degraded");
});

test("quota and backoff reasons become provider-backoff", () => {
  const state = deriveBrainRouteState(true, [{ capability: "assistant.respond", routable: false, providerReasonCode: "provider-quota-backoff" }]);
  assert.equal(state.kind, "provider-backoff");
  assert.equal(brainReadiness(state), "degraded");
});

test("assistant.respond must be routable before the brain is ready", () => {
  const state = deriveBrainRouteState(true, [{ capability: "assistant.respond", routable: true, enabled: true, provider: "zero-credit" }]);
...
test("a healthy answer route wins over an earlier degraded route", () => {
  const state = deriveBrainRouteState(true, [
    { capability: "assistant.respond", routable: false, enabled: true, provider: "licensed-question-model", providerReasonCode: "provider-quota-backoff" },
...
test("all-degraded routes preserve an actionable quota reason", () => {
  const state = deriveBrainRouteState(true, [
    { capability: "assistant.respond", routable: false, routingReason: "provider.gap" },
    { capability: "assistant.respond", routable: false, providerReasonCode: "provider-quota-backoff" },
  ]);
  assert.equal(state.kind, "provider-backoff");
  assert.equal(state.reason, "provider-quota-backoff");
});
...
test("disconnected transport remains offline regardless of stale capabilities", () => {
  const state = deriveBrainRouteState(false, [{ capability: "assistant.respond", routable: true }], "relay-disconnected");
