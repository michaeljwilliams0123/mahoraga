...
test("cloud workspace exposes the bounded Composio GitHub probe through the owner relay", async () => {
  const [routeText, relayText, viewText, workspaceText] = await Promise.all([
    readFile(route, "utf8"), readFile(relay, "utf8"), readFile(view, "utf8"), readFile(workspace, "utf8"),
  ]);
  assert.match(routeText, /"composio-github-repository"/);
  ...
  assert.doesNotMatch(routeText + relayText + viewText + workspaceText, /COMPOSIO_API_KEY/);
});
