...
export function CockpitView({
  ...
  const railwayStandby = deploymentProvider === "railway" || promotionMode === "exact-sha-railway";
  const deploymentDetail = railwayStandby
    ? `Railway is non-routing standby rollback only · not eligible for live traffic · ${deploymentEnvironment}`
    : deploymentProvider === "vercel"
      ? `non-authoritative preview - historical only - not production · ${deploymentEnvironment}`
      : `Cloudflare execution Worker is canonical production runtime · ${deploymentEnvironment}`;
  ...
  const noRailwayDetail = noRailwayVerified
    ? "Verified no Railway fallback on sanitized receipt; Railway is non-routing standby rollback only"
    : "Unproven; Railway stays non-routing standby rollback until noRailwayFallbackVerified is true";
    ...
    <section className="connection-panel eclipse-console" aria-label="Control Center">
      <header className="eclipse-header">
        <div>
          ...
          <p>Cloudflare execution Worker is the canonical production runtime for 7.0.0-alpha.2. Railway is non-routing standby rollback only. Execution readiness, cognition readiness, owner-browser/gateway, and custom-domain traffic remain separate gates. 7.0.0-alpha.2 is build provenance only. Windows 3.6.0 stays untouched.</p>
        </div>
      ...
      <p className="eclipse-readiness-note" role="status">
        Canonical runtime: Cloudflare execution Worker. Railway: non-routing standby. Browser: {CLOUDFLARE_WORKSPACE_CANDIDATE}. GitHub Pages remains a presentation/export fallback. /api/ready is observational only and never grants traffic/domain authority. Execution receipt does not prove owner-browser or custom-domain paths.
      </p>
      ...
      {!coreReady && !healthError && (
        <div className="eclipse-readiness-note" role="status">
          ...
          <div>
            ...
            <p>Presentation surfaces stay non-authoritative. Canonical execution is the accepted Cloudflare Worker. Railway stays standby rollback with no routing eligibility. Ready requires observed execution readiness plus pairing; traffic authority remains independently gated.</p>
          </div>
        </div>
      )}
      ...
      <div className="eclipse-status-grid">
        ...
        <StatusCard label="Deployment Truth" value={railwayStandby ? "Railway standby (no route)" : "Cloudflare canonical"} detail={`${deploymentConvergence} \u00b7 actual ${shortSha(deploymentCommit)} \u00b7 expected ${shortSha(expectedDeploymentCommit)}`} tone={deploymentConvergence === "Current" ? "good" : deploymentConvergence === "Drift" ? "warn" : "neutral"} />
        ...
        <StatusCard label="Canonical runtime" value="Cloudflare Worker" detail="Accepted execution Worker is production authority; Railway has no routing eligibility" tone="good" />
        <StatusCard label="Deployment" value={railwayStandby ? "Railway standby rollback" : health?.ok ? "Published" : "Awaiting health"} detail={deploymentDetail} tone={health?.ok && !railwayStandby ? "good" : health?.ok ? "neutral" : "warn"} />
        ...
        <StatusCard label="Expected SHA pin" value={expectedDeploymentCommit ? shortSha(expectedDeploymentCommit) : "Unset"} detail="MAHORAGA_EXPECTED_GIT_SHA \u00b7 pin the accepted Cloudflare execution Worker after exact-main live acceptance; Railway GitHub status is not routing authority. Wait for CI disabled." tone={expectedDeploymentCommit ? (deploymentConvergence === "Current" ? "good" : "warn") : "warn"} />
      ...
      <div className="eclipse-detail-grid">
        <section className="eclipse-panel" aria-labelledby="telemetry-heading">
          ...
          <dl className="eclipse-metrics">
            ...
            <div><dt>Host provider</dt><dd>{railwayStandby ? "railway (non-routing standby rollback)" : "cloudflare-execution-worker (canonical)"}</dd></div>
            ...
            <div><dt>Pin policy</dt><dd>MAHORAGA_EXPECTED_GIT_SHA \u00b7 after exact-main Cloudflare live acceptance; verified run SHA; Railway is standby only; Wait for CI disabled</dd></div>
            ...
            <div><dt>No Railway fallback</dt><dd>{noRailwayVerified ? "Verified" : "Unproven"} \u00b7 Railway non-routing standby \u00b7 x-bypass-applied fail-closed</dd></div>
        ...
        <section className="eclipse-panel" aria-labelledby="evolution-heading">
          ...
          <ol className="eclipse-flow">
            ...
            <li><span>4</span><div><strong>Pin</strong><small>Pin the accepted Cloudflare execution Worker to the verified exact-main SHA; keep Railway as non-routing standby</small></div></li>
