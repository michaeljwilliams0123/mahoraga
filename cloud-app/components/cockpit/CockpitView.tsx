...
function parseBillingAttestation(health: Health | null): {
  freePlanAttested: boolean;
  billableAccountSubscriptionCount: number | null;
  ...
  const nested = [
    ...
    asRecord(root?.billingAttestation),
    asRecord(root?.zeroCredit),
    asRecord(root?.cloudflareBilling),
    asRecord(root?.runtime),
    asRecord(asRecord(root?.runtime)?.billing),
  ].filter((value): value is Record<string, unknown> => value !== null);

  const countRaw = nested.map((entry) => entry.billableAccountSubscriptionCount).find((value) => typeof value === "number");
  const defaultUsageModel = firstString(...nested.map((entry) => entry.defaultUsageModel)) ?? null;
  const evidenceSource = firstString(...nested.map((entry) => entry.evidenceSource), ...nested.map((entry) => entry.billingEvidenceSource)) ?? null;
  const billableAccountSubscriptionCount = typeof countRaw === "number" && Number.isSafeInteger(countRaw) ? countRaw : null;
  const freePlanAttested = billableAccountSubscriptionCount === 0;
  return { freePlanAttested, billableAccountSubscriptionCount, defaultUsageModel, evidenceSource };
}
...
export function CockpitView({
  ...
  const billing = parseBillingAttestation(health);
  const billingValue = billing.freePlanAttested ? "Free (subscriptions evidence)" : "Fail-closed";
  const billingDetail = billing.freePlanAttested
    ? `billableAccountSubscriptionCount=0 \u00b7 evidence ${billing.evidenceSource ?? "subscriptions"} \u00b7 defaultUsageModel ${billing.defaultUsageModel ?? "unset"} is metadata only`
    : "Free plan unattested until subscriptions evidence proves billableAccountSubscriptionCount === 0; bundled/standard usage model is not billing authority";
  ...
  return (
    <section className="connection-panel eclipse-console" aria-label="Control Center">
      ...
      <div className="eclipse-status-grid">
        ...
        <StatusCard label="Cloudflare billing plan" value={billingValue} detail={billingDetail} tone={billing.freePlanAttested ? "good" : "warn"} />
      ...
      <div className="eclipse-detail-grid">
        <section className="eclipse-panel" aria-labelledby="telemetry-heading">
          ...
          <dl className="eclipse-metrics">
            ...
            <div><dt>Cloudflare billing plan</dt><dd>{billing.freePlanAttested ? "Free attested from subscriptions" : "fail-closed"} \u00b7 count {billing.billableAccountSubscriptionCount ?? "missing"}</dd></div>
            <div><dt>Usage model metadata</dt><dd>{billing.defaultUsageModel ?? "unset"} \u00b7 informational only \u00b7 never billing-plan authority</dd></div>
