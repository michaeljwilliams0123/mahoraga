...
const state = { repository: null, commit: null, issues: [], pulls: [], runs: [], draft: '' };
...
async function refreshCloudState(manual = false) {
  ...
  const responses = await Promise.allSettled([
    ...
    github('/pulls?state=open&per_page=30'), github('/actions/runs?per_page=12'),
  ]);
...
function renderCloudState(complete) {
  ...
  renderRows('task-list', state.issues.slice(0, 10).map((item) => ({ href: item.html_url, title: item.title, detail: `#${item.number} · updated ${relativeTime(item.updated_at)}`, state: item.state })));
...
async function openGithubTask() {
  const profile = $('tool-profile').selectedOptions[0].textContent;
  const returnMode = $('return-mode').value;
  const draft = state.draft ? `${state.draft}\n\nRequested tool profile: ${profile}\nRequested return mode: ${returnMode}` : '';
  if (draft) {
