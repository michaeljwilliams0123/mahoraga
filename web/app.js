...
function renderCapabilityRegistry() {
  $('capability-list').innerHTML = state.status.capabilities.map((item) => {
    ...
    return `<article class="control-card"><div class="card-head"><div><p class="eyebrow">${escapeHtml(item.interfaceType)} · ${escapeHtml(item.costClass)}</p><h3>${escapeHtml(item.capability)}</h3></div><span class="badge ${runnable ? 'ready' : 'disabled'}">${escapeHtml(routing)}</span></div><div class="evidence-grid capability-evidence"><span>Runtime<b>${escapeHtml(item.process)}</b></span><span>Provider<b>${escapeHtml(item.provider)}</b></span><span>Canary<b>${escapeHtml(item.canary)}</b></span><span>Routing<b>${escapeHtml(routing)}</b></span><span>Evidence<b>${escapeHtml(item.evidenceLevel)}</b></span></div><p>${escapeHtml(item.workerLabel)} · ${escapeHtml(item.permissionClass)}</p><small>Verified ${formatTime(item.lastVerifiedAt)} · observed ${formatTime(item.lastObservedAt)} · fallback ${escapeHtml(fallbacks)}</small><div class="card-actions"><button data-capability-action="${escapeHtml(item.capability)}" ${runnable ? '' : 'disabled'} title="${escapeHtml(routing)}">Run capability</button></div></article>`;
  }).join('');
}
