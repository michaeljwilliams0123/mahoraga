...
const IDENTITY_KEYS = new Set(["schemaVersion", "product", "buildVersion"]);
...
export function validateProductIdentity(value) {
  ...
  if (value.schemaVersion !== 2 || value.product !== "Mahoraga") throw new TypeError("product-identity-invalid");
  if (typeof value.buildVersion !== "string" || value.buildVersion.length > 40 || !VERSION_PATTERN.test(value.buildVersion)) throw new TypeError("product-build-version-invalid");
  return Object.freeze({ schemaVersion: 2, product: value.product, buildVersion: value.buildVersion });
}
...
export function assertProductIdentityMirrors(identity, mirrors) {
  ...
  for (const [surface, version] of Object.entries(mirrors)) {
    ...
    if (version !== canonical.buildVersion) throw new TypeError(`product-build-version-divergence:${surface}`);
