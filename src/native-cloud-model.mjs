...
const ZERO_CREDIT_COST_CLASS = Object.freeze({ "codespaces-open-weight": "cloud-open-weight", "local-open-weight": "local-model" });
...
export async function probeNativeCloudModel({ env = process.env } = {}) {
  ...
  return { verified: true, summary: "Native cloud GPT-5.6 Sol is bound through the Vercel AI Gateway.", providerHealth: health("healthy") };
}
...
export async function probeZeroCreditAnswerModel({ providerId = "codespaces-open-weight", env = process.env } = {}) {
  ...
  if (!evidence.ok) return zeroCreditUnavailable(evidence.reasonCode, providerDecision);
  return Object.freeze({
    verified: true,
    summary: `Zero-credit answer provider ${providerId} has complete server-side configuration and verified zero-dollar admission evidence.`,
    externalProviderCalls: 0,
    providerDecision,
    providerHealth: Object.freeze({
      availability: "healthy", provider: providerId, model: configuration.model, executionMode: "zero-credit-openai-compatible",
      networkAccess: true, zeroCreditVerified: true, billingState: evidence.value.billingState, metered: evidence.value.metered,
      priceUsd: evidence.value.priceUsd, spendUsd: evidence.value.spendUsd, zeroDollarStopGuaranteed: evidence.value.zeroDollarStopGuaranteed,
      capabilityCanary: evidence.value.capabilityCanary, responseContentPersistedOutsideVault: false,
...
export async function executeNativeCloudModel({ task, fetchImpl = fetch, env = process.env } = {}) {
  ...
  try {
    response = await fetchImpl(GATEWAY_URL, {
      ...
      body: JSON.stringify({ model: MODEL, messages: [{ role: "user", content: prompt }], stream: false }), signal: AbortSignal.timeout(55_000),
    });
  } catch { return unavailable("ai-gateway-request-failed"); }
  ...
  const usage = { inputTokens: boundedCount(payload?.usage?.prompt_tokens), outputTokens: boundedCount(payload?.usage?.completion_tokens) };
  return { verified: true, answer: answer.trim(), summary: answer.trim().slice(0, 512), providerHealth: { ...health("healthy"), usage } };
}
...
export async function executeZeroCreditAnswerModel({ task, authorityDecision, providerDecision, providerEvidence, fetchImpl = fetch, env = process.env } = {}) {
  ...
  try {
    response = await fetchImpl(configuration.url, {
      method: "POST", headers: zeroCreditHeaders(configuration.token),
      body: JSON.stringify({ model: configuration.model, messages: [{ role: "user", content: prompt }], stream: false }), signal: AbortSignal.timeout(55_000),
    });
  } catch { return zeroCreditUnavailable("zero-credit-model-request-failed", providerDecision); }
  ...
  const answer = payload?.choices?.[0]?.message?.content;
  if (typeof answer !== "string" || answer.trim().length < 1 || answer.length > MAX_RESPONSE_CHARS) return zeroCreditUnavailable("zero-credit-model-answer-invalid", providerDecision);
  return Object.freeze({
    verified: true, taskId: boundedIdentifier(task?.id), correlationId: boundedIdentifier(task?.correlationId), answer: answer.trim(), summary: answer.trim().slice(0, 512), externalProviderCalls: 1,
    providerDecision: Object.freeze({ status: "selected", providerId: providerDecision.providerId, costClass: providerDecision.costClass }),
    providerHealth: Object.freeze({
      availability: "healthy", provider: providerDecision.providerId, model: configuration.model, executionMode: "zero-credit-openai-compatible", networkAccess: true,
      zeroCreditVerified: true, billingState: evidence.billingState, metered: evidence.metered, priceUsd: evidence.priceUsd, spendUsd: evidence.spendUsd,
      zeroDollarStopGuaranteed: evidence.zeroDollarStopGuaranteed, responseContentPersistedOutsideVault: false,
      usage: Object.freeze({ inputTokens: boundedCount(payload?.usage?.prompt_tokens), outputTokens: boundedCount(payload?.usage?.completion_tokens) }),
    }),
  });
}
...
export function zeroCreditProviderEvidenceFromEnv({ providerId = "codespaces-open-weight", env = process.env } = {}) {
  ...
  const billingState = String(env?.MAHORAGA_ZERO_CREDIT_BILLING_STATE ?? "").trim();
  const stopGuaranteed = String(env?.MAHORAGA_ZERO_CREDIT_ZERO_DOLLAR_STOP_GUARANTEED ?? "").trim().toLowerCase();
  if (metered !== "false" || priceUsd !== "0" || spendUsd !== "0") return rejected("zero-credit-provider-not-zero-dollar");
  if (billingState !== "verified-zero" || stopGuaranteed !== "true") return rejected("zero-credit-provider-billing-unverified");
  return Object.freeze({ ok: true, reasonCode: null, value: Object.freeze({ id: providerId, metered: false, priceUsd: 0, spendUsd: 0, billingState: "verified-zero", zeroDollarStopGuaranteed: true, ready: true, capabilityCanary: Object.freeze({ fresh: true }) }) });
}

function readCredential(env) { return typeof env?.AI_GATEWAY_API_KEY === "string" ? env.AI_GATEWAY_API_KEY.trim() : ""; }
...
function zeroCreditHeaders(token) { const headers = { "content-type": "application/json" }; if (token) headers.authorization = `Bearer ${token}`; return headers; }
function health(availability, reasonCode) {
  const value = { availability, provider: "vercel-ai-gateway", model: MODEL, executionMode: "core-routed-native-api", networkAccess: true, responseContentPersistedOutsideVault: false, usage: { inputTokens: 0, outputTokens: 0 } };
  if (reasonCode) value.reasonCode = reasonCode;
  return value;
}
function unavailable(reasonCode) { return { verified: false, summary: `Native cloud answer unavailable: ${reasonCode}.`, providerHealth: health("unavailable", reasonCode) }; }
function zeroCreditUnavailable(reasonCode, providerDecision = null) {
  return Object.freeze({
    verified: false, summary: `Zero-credit answer unavailable: ${reasonCode}.`, externalProviderCalls: 0,
    providerDecision: providerDecision?.providerId ? Object.freeze({ status: providerDecision.status ?? null, providerId: providerDecision.providerId, costClass: providerDecision.costClass ?? null }) : null,
    providerHealth: Object.freeze({ availability: "unavailable", provider: providerDecision?.providerId ?? "zero-credit-answer", executionMode: "zero-credit-openai-compatible", zeroCreditVerified: false, reasonCode, responseContentPersistedOutsideVault: false, usage: Object.freeze({ inputTokens: 0, outputTokens: 0 }) }),
  });
}
