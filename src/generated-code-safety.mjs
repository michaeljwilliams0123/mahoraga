...
function inspectJavaScript(source, reasons) {
  ...
  if (/node:(?:child_process|cluster|worker_threads)|\b(?:child_process|Bun\.spawn|Deno\.Command|process\.(?:spawn|exec|fork|kill))\b/.test(source)) reasons.add("process-access");
  if (/node:(?:net|tls|dgram|http|https|http2)|\b(?:WebSocket|EventSource|fetch)\s*\(/.test(source)) reasons.add("network-access");
  if (/\bprocess\.env\b|\bDeno\.env\b|\bimport\.meta\.env\b/.test(source)) reasons.add("environment-access");
  if (/node:fs|\brequire\s*\(\s*["']fs["']/.test(source)) reasons.add("unrestricted-filesystem");
}
