...
export function classifyTaskIntent({ content = "", attachmentCount = 0, availableCapabilities = [] } = {}) {
  ...
  else if (matches(text, /browser.*\b(health|smoke|status)\b|\b(smoke|health)\b.*browser/)) result = route("browser-health", choose(available, "browser.status", "browser.smoke"), "browser-health-check", "evidence.browser-health");
...
export function validateIntentDecision(value) {
  ...
  if (value.capability !== null && (!CAPABILITIES.has(value.capability) || value.intentKind === "unsupported")) throw new TypeError("Intent capability is invalid.");
...
function isMicrosoftWork(text) { return /https?:\/\/[^\s/]+\.(sharepoint\.com|microsoft\.com|office\.com)\b/.test(text) || /\b(sharepoint|microsoft 365|dataverse|teams)\b/.test(text); }
