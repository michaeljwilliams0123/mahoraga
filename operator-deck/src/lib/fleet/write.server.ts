...
async function mergePr(number: number): Promise<WriteOutcome> {
  ...
  if (meta.mergeStateStatus && meta.mergeStateStatus !== "CLEAN") {
    return {
      ...
      summary: `#${number} “${meta.title ?? ""}” is ${meta.mergeStateStatus}. Protect main requires exact-head Verify (Ubuntu + Windows). Retired Vercel statuses are non-authoritative and must not gate merge. This deck will not squash past a blocked ruleset.`,
      evidence: [
