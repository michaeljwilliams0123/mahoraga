...
export async function buildGithubAudit({ root = ROOT, listTrackedFiles = trackedFiles } = {}) {
  ...
  const workspaceAgentReceiverTrusted = workspaceAgentReceiverMissing.length === 0
    ...
    && /options:\s*\[destiny-workspace, mike-primary, balanced, overflow\]/.test(workspaceAgentReceiver)
    && /DESTINY_WORKSPACE_AGENT_ACCESS_TOKEN:\s*\$\{\{ secrets\.DESTINY_WORKSPACE_AGENT_ACCESS_TOKEN \}\}/.test(workspaceAgentReceiver)
    && /DESTINY_WORKSPACE_AGENT_TRIGGER_ID:\s*\$\{\{ secrets\.DESTINY_WORKSPACE_AGENT_TRIGGER_ID \}\}/.test(workspaceAgentReceiver)
    && /DESTINY_WORKSPACE_RECEIPT_PUBLIC_KEY_SPKI:\s*\$\{\{ secrets\.DESTINY_WORKSPACE_RECEIPT_PUBLIC_KEY_SPKI \}\}/.test(workspaceAgentReceiver)
    && /DESTINY_WORKSPACE_RECEIPT_PUBLIC_KEY_FINGERPRINT:\s*\$\{\{ vars\.DESTINY_WORKSPACE_RECEIPT_PUBLIC_KEY_FINGERPRINT \}\}/.test(workspaceAgentReceiver)
    && /DESTINY_WORKSPACE_RECEIPT_KEY_ID:\s*\$\{\{ vars\.DESTINY_WORKSPACE_RECEIPT_KEY_ID \}\}/.test(workspaceAgentReceiver)
    && /MIKE_PRIMARY_AGENT_ACCESS_TOKEN:\s*\$\{\{ secrets\.MIKE_PRIMARY_AGENT_ACCESS_TOKEN \}\}/.test(workspaceAgentReceiver)
    && /MIKE_PRIMARY_WORKSPACE_AGENT_TRIGGER_ID:\s*\$\{\{ secrets\.MIKE_PRIMARY_WORKSPACE_AGENT_TRIGGER_ID \}\}/.test(workspaceAgentReceiver)
