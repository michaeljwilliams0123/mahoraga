...
const SUSPICIOUS_PATH = /(?:^|\/)(?:\.git(?:$|\/)|\.env(?:\..*)?|\.ssh(?:$|\/)|\.aws(?:$|\/)|\.config\/gcloud(?:$|\/)|id_(?:rsa|dsa|ecdsa|ed25519)|credentials?|secrets?|tokens?|.*\.(?:pem|p12|pfx|key))(?:$|\/)/i;
const SECRET_CONTENT = /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\bAKIA[0-9A-Z]{16}\b|\b(?:gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|sk-[A-Za-z0-9]{20,})\b|\b(?:api[_-]?key|access[_-]?token|auth[_-]?token|client[_-]?secret|password|secret|token)\s*[:=]\s*["']?[A-Za-z0-9_\-/.]{16,}["']?/i;
...
async function collect(root, relative, output) {
  if (isExcludedTree(relative)) return;
  ...
  for (const entry of (await readdir(resolved, { withFileTypes: true })).sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (isExcludedTree(child)) continue;
    await collect(root, child, output);
  }
}
function isExcludedTree(relative) {
  return relative.split("/").some((part, index, parts) => part === ".git" || part === ".ssh" || part === ".aws" || (part === ".config" && parts[index + 1] === "gcloud"));
}
