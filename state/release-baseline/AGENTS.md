...
- Distributed task transport is at-least-once. Safe effects come from immutable request hashes, idempotent transitions, expiring leases, monotonically increasing fencing tokens, and stale-writer rejection; never claim literal exactly-once transport.
- Private deployment identities belong in ignored runtime configuration. Public repository audits must detect environment names, tenant identifiers, provider endpoints, personal paths, and credentials without printing matched values.
- Copilot custom agents are explicit owner-selected specialists. They may return pull requests but may not merge, release, alter visibility, activate updates, or access content outside the repository task envelope.
