...
def main():
    ...
    expired = []
    for expired_state in ("Claimed", "Running"):
        expired.extend(list(client.records.list(
            table,
            filter=f"{prefix}_state eq '{expired_state}' and {prefix}_leaseexpiresat lt {now.isoformat()}",
            select=[primary_id], top=25,
        )))
    for row in expired:
        client.records.update(table, row[primary_id], {
            f"{prefix}_state": "Queued", f"{prefix}_relayid": None,
            f"{prefix}_localtaskkey": None, f"{prefix}_errorcode": "lease-expired-requeued",
        })
    ...
    for row in running:
        ...
        state = {
            ...
            "waiting": "Waiting", "waiting_for_user": "Waiting",
        }[local["status"]]
        client.records.update(table, row[primary_id], {
    ...
    print(json.dumps({"verified": True, "claimed": claimed, "completed": completed, "requeued": len(expired), "relay": relay}))
