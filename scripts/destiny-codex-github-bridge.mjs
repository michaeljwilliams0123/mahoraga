...
const route = JSON.parse(await readFile(path.join(stateDir, "route-private.json"), "utf8"));
if (route?.schemaVersion !== 1 || route.kind !== "destiny-codex-private-route" || route.repository !== REPOSITORY || typeof route.environmentId !== "string") {
  throw new Error("destiny-codex-private-route-invalid");
}
...
const issue = options.has("issue-file")
  ? JSON.parse(await readFile(path.resolve(options.get("issue-file")), "utf8"))
...
if (ledger[task.taskId]) {
  console.log(JSON.stringify({ submitted: false, duplicate: true, issueNumber, taskId: task.taskId, taskUrl: ledger[task.taskId].taskUrl ?? null }));
  process.exit(0);
}
...
let stdout;
try {
  stdout = execFileSync(executable, buildCodexCloudExecArgs(route.environmentId, task), {
    encoding: "utf8",
    windowsHide: true,
...
console.log(JSON.stringify({ submitted: true, duplicate: false, issueNumber, taskId: task.taskId, taskUrl }));

async function fetchIssue(number) {
  const response = await fetch(`https://api.github.com/repos/${REPOSITORY}/issues/${number}`, {
    headers: { Accept: "application/vnd.github+json", "User-Agent": "mahoraga-destiny-codex-bridge" },
  });
...
async function readLedger(file) {
  try {
    ...
    throw new Error("destiny-codex-submit-ledger-invalid");
  }
}
